...
void Polyhedron::render(Scene& scene, BoundingBox& boundingBox, Dimension toIgnore) const {
  ...
  //This implementation doesn't use the flattened normalized vertices
  for (const auto& vertex : vertices) {
    RGB normalizedColor = scene.normalize(vertex.color);

    switch (toIgnore) {
      case Z:
        draw_pix((int) vertex.x(), (int) vertex.y(), normalizedColor.r, normalizedColor.g, normalizedColor.b);
      case Y:
        draw_pix((int) vertex.x(), (int) vertex.z(), normalizedColor.r, normalizedColor.g, normalizedColor.b);
      case X:
        draw_pix((int) vertex.y(), (int) vertex.z(), normalizedColor.r, normalizedColor.g, normalizedColor.b);
    }
  }
